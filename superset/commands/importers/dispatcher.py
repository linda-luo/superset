...
class ImportDispatcherCommand(BaseCommand):
    """
    Base command that dispatches an import to different versions of the
    command until it finds one that can handle the contents.

    Subclasses only need to define ``command_versions`` with the ordered list
